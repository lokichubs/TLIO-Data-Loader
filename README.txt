To properly format any IMU data ensure to follow the process below:
...
2. Run the "groundTruthFormat.ipynb" to convert the MOCAP to the TLIO format
...
The above should create a folder named "FORMATTED DATA" that ensures all the IMU and MOCAP data is formatted
as needed by the TLIO. Copy this into the instace whether the "TLIO~master" is on your device and move it to
the "local_data" file. Then it should be ready to train the model.
